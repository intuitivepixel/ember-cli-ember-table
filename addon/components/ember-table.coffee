...
EmberTableComponent = Ember.Component.extend StyleBindingsMixin, ResizeHandlerMixin,
  ...
  object. See `ColumnDefinition.getCellContent`.
  ###
  content: null

  ###
  An array of column definitions: see `ColumnDefinition`. Allows
  ...
  ### An array of Row computed based on `content` ###
  bodyContent: Ember.computed ->
    RowArrayController.create
      target: this
